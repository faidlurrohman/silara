...
export default function RekeningJenis() {
  ...
  const columns = [
    {
      ...
      width: 200,
      sorter: (a, b) =>
        a.account_type_label.localeCompare(b.account_type_label),
      sortOrder:
        sorted.columnKey === "account_type_label" ? sorted.order : null,
      ...getColumnSearchProps("account_type_label", "Jenis Rekening"),
    },
    {
      title: "Label",
      dataIndex: "label",
  ...
  const fetchDataAccount = () => {
    setLoading(true);
    getAccount("object").then((response) => {
      setLoading(false);
  ...
  const fetchDataAccountType = () => {
    setLoadingType(true);
    getAccountList("type").then((response) => {
  ...
  const deleteRow = (values) => {
    modal.warning({
      ...
      okCancel: true,
      onOk() {
        removeAccount("object", values?.id).then(() => {
  ...
  return (
    <>
      <div className="flex flex-row space-x-2">
        <CSVLink
          data={accountObject.map(
            ({ account_type_label, label, remark, active }) => ({
              account_type_label,
              label,
              remark,
