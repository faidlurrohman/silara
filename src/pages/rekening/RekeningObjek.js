...
import { REKENING_OBJEK_TMP } from "../../helpers/constants";

export default function RekeningObjek() {
  ...
  return (
    <>
      <div className="flex flex-row space-x-2">
        <CSVLink
          data={REKENING_OBJEK_TMP.map(({ type, label, remark, isActive }) => ({
            type,
            label,
            remark,
          ...
          headers={[
            { label: "Jenis Rekening", key: "type" },
            { label: "Label", key: "label" },
            { label: "Keterangan", key: "remark" },
            { label: "Aktif", key: "isActive" },
          ]}
          filename={"DATA_REKENING_OBJEK.csv"}
        >
      ...
      <div className="mt-4">
        <Table dataSource={REKENING_OBJEK_TMP} columns={columns} />
      </div>
      <Modal
        centered
        open={isShow}
        title={`${isEdit ? `Ubah` : `Tambah`} Rekening Objek`}
      ...
      >
        ...
        >
          ...
          <Form.Item label="Jenis Rekening" name="type">
            <Select />
          </Form.Item>
