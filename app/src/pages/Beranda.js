...
import { Card, DatePicker, Progress, Select, Space, Statistic } from "antd";
import {
	COLORS,
	DATE_FORMAT_VIEW,
	MONTHS,
...
const config = {
	...
	color: [COLORS.infoPastel, COLORS.successPastel, COLORS.dangerPastel],
	point: {
		shape: ({ category }) => {
...
export default function Beranda() {
		...
		axios
			...
			.then(
				axios.spread((_data, _recap, _cities) => {
					...
					makeGraph(responseGet(_recap).data);
				})
			);
	};
	...
	const makeGraph = (values) => {
		...
		let _ly = parseInt(convertDate(convertDate(), "YYYY"));

		// ambil 3 tahun dari tahun sekarang
		let _fy = _ly - 2;
		...
		for (let a = 0; a < 3; a++) {
			...
			for (let b = 0; b < MONTHS.length; b++) {
				_g.push({
					month_in_year: `${MONTHS[b]} (${String(_fy)})`,
					year: String(_fy),
					month: MONTHS[b],
					month_index: b + 1 < 10 ? `0${b + 1}` : String(b + 1),
				});
			}
				...
				>
					...
					<div className="h-auto w-auto pt-2">
						<Progress
							...
							size={[, 25]}
							strokeColor={COLORS.successPastel}
							status="normal"
						/>
					</div>
				</Card>
				...
				>
					...
					<div className="h-auto w-auto pt-2">
						<Progress
							...
							size={[, 25]}
							strokeColor={COLORS.dangerPastel}
							status="normal"
						/>
					</div>
				</Card>
			</div>
			<div className="flex mx-0.5 pb-2 hidden">
				<Card
					size="small"
					title={
						<span className="text-xs">
							GRAFIK PENDAPATAN, PEMBIAYAAN DAN BELANJA DAERAH 3 TAHUN TERAKHIR
						</span>
					}
					className="text-center w-full"
				>
					<Line {...config} data={recap} loading={loading} />
				</Card>
			</div>
		</>
	);
