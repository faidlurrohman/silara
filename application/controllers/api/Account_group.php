...
class Account_group extends REST_Controller {
    ...
    {   
        ...
        if($user){
            ...
            $data = $this->Account_group_model->get_all($user, $limit, $offset, $order, $filter);
    ...
    {   
        ...
        if($user){
            $data = $this->Account_group_model->get_list($user);
    ...
    private function do_create()
    {
        ...
        if($user){
            $data = $this->Account_group_model->save($user, $this->input_fields());
    ...
    private function do_delete($id)
    {
        ...
        if($user){
            $data = $this->Account_group_model->delete($user, array('id' => $id));
